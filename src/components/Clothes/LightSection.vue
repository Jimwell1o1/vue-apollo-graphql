<template>
    <div v-if="loading" class="justify-center flex flex-wrap">
        <div class="m-5">
    ...
    <section v-else-if="(result && result.categories)" id="categories" class="flex gap-20 justify-center items-center h-max py-20 text-white font-medium">
        <div class="grid place-items-center h-32 w-32 rounded-full bg-blend-overlay bg-gray-800/75 bg-cover border-green-400 border-[3px] hover:bg-gray-900/75 hover:text-green-300 transition-all ease-in cursor-pointer bg-all-products"
            @click.prevent="selectCategory('all')">
            All Products
        </div>
        <div v-for="category of result.categories" :key="category.id">
            <div class="grid place-items-center h-32 w-32 rounded-full bg-blend-overlay bg-gray-800/75 bg-cover border-green-400 border-[3px] hover:bg-gray-900/75 hover:text-green-300 transition-all ease-in cursor-pointer" 
                :style="`background-image: url('http://127.0.0.1:8000/images/categories/${category.img_path}')`"
                @click.prevent="selectCategory(category.category_name)">
                {{ category.title }}
            </div>
    ...
    <div v-else-if="error2" id="alert-5" class="flex p-4 m-5 bg-gray-100 rounded-lg dark:bg-gray-700 w-5/12" role="alert">
      ...
      <div v-else-if="(selectedCategory === 'all' && data2 && data2.lights)" class="flex flex-wrap justify-center gap-5 max-w-7xl mb-20">
        <div v-for="light of data2.lights" :key="light.id"  class="max-w-[18rem] h-full max-h-[405px] rounded-md overflow-hidden shadow-lg">
      ...
      <div v-else class="flex flex-wrap justify-center gap-5 max-w-7xl mb-20">
           <LightCategorized :category="selectedCategory"/>
      </div> 
...
<script lang="ts" setup>
    ...
    let queryMultiple = () => {
            const allProducts = useQuery(lightsQuery);
            return [allProducts];
        }
    const [
